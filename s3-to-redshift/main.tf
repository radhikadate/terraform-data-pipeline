...
module "s3_notification" {
  ...
  bucket_name    = data.terraform_remote_state.static_setup.outputs.manifest_bucket_name
...
module "redshift_autocopy" {
  ...
  s3_source_path                = "s3://${data.terraform_remote_state.static_setup.outputs.manifest_bucket_name}/manifest/"
